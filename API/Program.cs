...
{
    if(ctx.Tarefas.Status(id) == "Não iniciada")
    {
        ctx.Tarefas.Status(id)="Em andamento";
    }else
    {
        ctx.Tarefas.Status(id)="Concluída";
    }
    ctx.SaveChanges();
    ctx.Tarefas.Update();
    return Results.Ok();
...
app.MapGet("/api/tarefas/naoconcluidas", ([FromServices] AppDataContext ctx) =>
{
   if (ctx.Tarefas.Any())
    {
        List<Tarefa> lista = ctx.Tarefas.Where(tarefa => tarefa.Status == "Não iniciada").ToList();
